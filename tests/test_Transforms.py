...
import unittest
import random
from MomentMatching.baseMomentMatch import UnscentedTransform, MonteCarloTransform, TaylorTransform
...
class TestFunctions:
    ...
    def linear(self, x, t=None):
        y = self.A @ x + self.B
        return y

    def sinusoidal(self, x, t=None):
        y = np.sin(0.5 * x)
        return 2 * y

    def softplus(self, x, t=None):
...
class TestTaylorTransform(unittest.TestCase):
    def setUp(self):
        ...
        self.TT = TaylorTransform(dimension_of_state=self.dim)
    ...
    def test_predict_linear(self):
        result_under_test = self.TT.predict(self.test_functions.linear, self.x_state )
    ...
    def test_predict_sinusoidal(self):
        result_under_test = self.TT.predict(self.test_functions.sinusoidal, self.x_state)
        pred_mean, pred_cov, pred_cross_cov = result_under_test
    ...
    def test_predict_softplus(self):
        result_under_test = self.TT.predict(self.test_functions.softplus, self.x_state)
        pred_mean, pred_cov, pred_cross_cov = result_under_test
        np.testing.assert_allclose(pred_mean, self.mean)
